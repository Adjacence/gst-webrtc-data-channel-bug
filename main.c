...
static void
_on_data_channel (GstElement * webrtc, GstWebRTCDataChannel * channel,
    gpointer user_data)
{
  g_print ("Got data channel");
}
...
main (int argc, char *argv[])
{
  ...
  GstElement *sendbin, *rtp_caps_filter;

  gst_init (&argc, &argv);

  loop = g_main_loop_new (NULL, FALSE);

  webrtc1 = gst_element_factory_make ("webrtcbin", "send");
  g_signal_connect (webrtc1, "on-negotiation-needed",
      G_CALLBACK (_on_negotiation_needed), NULL);
  webrtc2 = gst_element_factory_make ("webrtcbin", "recv");
  ...
  sendbin =
      ...
      ("videotestsrc ! video/x-raw,framerate=1/1 ! queue ! vp8enc ! rtpvp8pay ! queue ! "
       "capsfilter name=rtp_caps_filter "
  ...
  gst_bin_add_many (GST_BIN (pipe1), sendbin, webrtc1, webrtc2, NULL);
  ...
  gst_object_unref (rtp_caps_filter);
